...
needing your own input file.`,
  ...
  shingan analyze --format adk-go --input ./agents/ --since main`,
	...
	cmd.Flags().StringVar(&flags.format, "format", "json", "Input format: json, adk-go, samurai, langgraph, n8n, crewai, langgraph-js, pydantic-graph, llamaindex, or autogen")
...
func loadAsMulti(path, inputFormat string, p application.WorkflowParser, allow []string) ([]application.GraphWithSource, error) {
	...
	default:
		return nil, fmt.Errorf("directory input is only supported for adk-go, langgraph, crewai, langgraph-js, pydantic-graph, llamaindex, and autogen formats; use a single JSON file for json/samurai/n8n formats")
	}
