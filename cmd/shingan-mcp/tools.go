...
type ExplainRuleArgs struct {
	RuleName string `json:"rule_name" jsonschema:"one of the 20 built-in rule names (e.g. loop_guard, cycle_detection)"`
}
...
func registerTools(server *mcp.Server, deps *toolDeps) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Run all 20 Shingan analysis rules against an in-memory " +
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Return a detailed explanation of one of the 20 built-in Shingan rules.",
	}, deps.explainRule)
