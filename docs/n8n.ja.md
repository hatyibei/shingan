...
| ファイル | パターン | 期待される findings |
|---|---|---|
| `simple_chain.json` | Webhook → OpenAI → HTTP Request | `error_handler_checker` (Warning) ×2 |
| `branching.json` | OpenAI → IF → (HTTP / HTTP) | 各終端 HTTP に `error_handler_checker` (Warning) |
| `loop.json` | Webhook → Code → IF (ループバック) | `loop_guard` (Warning) — `if` で抜けるが max-iter 未設定 |
| `multi_step.json` | 5 ノード直列パイプライン (trigger → 4 tools) | クリーン |
| `ai_agent.json` | langchain `aiAgent` (sub-connections あり) | langchain sub-tools は skip、`main` フローのみ解析 |
